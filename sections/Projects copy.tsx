...
export const ProjectsSection = () => {
  ...
  const headerRef = useRef(null); // Ref for magnetic effect
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      if (header) {
        ...
        const radius = 300; // Area of influence
        const force = 0.6;  // Strength of the pull
  ...
  return (
    <div className="w-full h-full bg-black text-white overflow-hidden relative flex flex-col justify-center">
      
      {/* Background Spotlight */}
      <div 
        className="absolute inset-0 pointer-events-none opacity-30 transition-opacity duration-1000"
        style={{
          background: `radial-gradient(800px circle at ${mousePos.x}% ${mousePos.y}%, rgba(79, 70, 229, 0.12), transparent 80%)`
        }}
      />
      ...
      <div className="w-full mt-12 mb-10 px-10 md:px-20 relative z-10">
        <div className="max-w-[1800px] mx-auto flex flex-col md:flex-row md:items-end justify-between gap-10">
          <div className="space-y-4">
            <p className="text-[10px] font-black tracking-[0.8em] uppercase text-indigo-500">
              Case Studies
            </p>
            ...
            <h2 
              ref={headerRef}
              className="text-7xl md:text-[10rem] font-condensed uppercase leading-[0.8] tracking-tighter select-none transition-transform duration-700 ease-out pointer-events-auto"
              style={{ 
                transform: `translate3d(${headerOffset.x}px, ${headerOffset.y}px, 0)`,
                // Adding a slight glow that follows the intensity
                textShadow: headerOffset.x !== 0 ? `0 0 30px rgba(99, 102, 241, 0.2)` : 'none'
              }}
            >
      ...
      {/* Scroller */}
      <div ref={scrollRef} className="relative w-full flex overflow-x-hidden no-scrollbar select-none active:cursor-grabbing z-10">
          ...
          {infiniteProjects.map((project, idx) => {
            ...
            return (
              <div 
                ...
                `}
              >
                <img src={project.image} alt={project.title} className="absolute inset-0 w-full h-full object-cover opacity-30 group-hover:opacity-70 group-hover:scale-105 transition-all duration-1000 grayscale group-hover:grayscale-0" />
